...
if __name__=='__main__':
    ...
    option= 'crack3' #crack1/2/3, frost 1/2/3
    ...
    # posenet = DeepVONet().cuda()
    # resultModelFile = torch.load("/mnt/usb0/shyoon/ai28/RT_code/analysis/experiment_12_lr1e-05_batch8/logs/batchLosses/20210426-145512/result_12")
    # resultModelFile = torch.load("/mnt/usb0/shyoon/ai28/RT_code/analysis/best/logs/batchLosses/20210427-001945/result_12")
    posenet = DeepFMatResNet18(6, 'ETR').cuda()  # DeepVONet().cuda()
    resultModelFile = torch.load(
        "/mnt/usb0/shyoon/ai28/RT_code/analysis/experiment_12_lr0.0001_batch8/logs/batchLosses/20210427-173724/result_12")
    ...
    for i in range(len_of_img):
        if i >40 and i<110:
            ...
            # pdb.set_trace()
            # rot.from_euler()
            # temp = RT[0].detach().cpu().numpy()
            # mat = torch.from_numpy(eul2rot([temp[2],temp[0],temp[1]]))
            # mat=torch.from_numpy(eul2rot(RT[0].detach().cpu().numpy()[:3]))
            # print(mat)

            cur_trans = RT[0].detach().cpu().numpy()
            cur_trans_mat = torch.zeros((3, 4))
            # for j in range(3):
            #     cur_trans_mat[j,j]= float(cur_trans[j])
            ...
            pose_error = torch.norm(cur_trans_mat[0,:3,:3]-torch.eye(3))
