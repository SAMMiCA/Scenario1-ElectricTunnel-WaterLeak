...
if __name__=='__main__':
    ...
    option= '0610' #crack1/2/3, frost 1/2/3
...
########################################
        ...
        roll_new = -np.array(gt_R_euler[2])#*np.pi/180 #201
        pitch_new = -np.array(gt_R_euler[0])#*np.pi/180
        yaw_new = -np.array(gt_R_euler[1])#*np.pi/180

        new_euler = np.array([roll_new,pitch_new,yaw_new])

        _new_euler = scirot.from_euler('xyz',new_euler,degrees=True)
        new_trans_mat = _new_euler.as_matrix()
        ...
        cur_trans_mat[0, :, 3] = torch.Tensor([-0.25, 0.0, 0])
...
##################################################
        ...
        ######REMOVE
        cl = np.ones((576,576))
        cl[300:500,100:300] = np.zeros((200,200))
