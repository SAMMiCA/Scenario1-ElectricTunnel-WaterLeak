...
def vo(left_path, right_path, ldepth_path, rdepth_path, K):
    ...
    imageR = cv2.imread(right_path + '//' + str(i) + '_0.png')
...
def vo_temp(past_path, cur_path, pdepth_path, cdepth_path, K):

    imageP = cv2.imread(past_path)
    imageC = cv2.imread(cur_path + '//' + str(i) + '_1.png')
    ...
    depthP = imageio.imread(pdepth_path)
    depthC = imageio.imread(cdepth_path + '//' + str(i) + '_3.pfm')


    keypoint1, keypoint2 = feature_extraction.feature(imageP,imageC)

    # print("Before RANSAC", len(keypoint1), len(keypoint2))

    p1, p2 = coordinate.coordinate(keypoint1,keypoint2,depthP,depthC,K)
    p1, p2 = RANSAC.ransac(p1,p2)
...
if __name__=='__main__':
    ...
    for i in range(10,len_of_img):
        ...
        gt_R_Temp = scirot.from_matrix(gt_R)
        gt_R_euler = gt_R_Temp.as_euler('xyz', degrees=True)
        # print(gt_R_euler)
        ...
        cur_right_depth_th = toTensor(cur_right_depth) #right_depth

        # For pose estimation network
        # print(cur_right_normal_filename)

        rotation = vo(crack_dir_left,normal_dir_right,
                      depth_dir_left,depth_dir_right,K)
        ...
        if i>=10:
            ...
            past_left_depth_th = toTensor(past_left_depth)

            rotation_p2c= vo_temp(past_left_normal_filename,crack_dir_left,past_left_depth_filename,depth_dir_left,K)
...
########################################
        ...
        #Temporal axis
        cur_trans_mat_TA = torch.zeros((3, 4))

        rot_pred = scirot.from_matrix(rotation_p2c)
        ...
        cur_trans_mat_TA = cur_trans_mat_TA.unsqueeze(0)
        cur_trans_mat_TA[:, :3, :3] = rot

        cur_trans_mat_TA[0, :, 3] = torch.Tensor([0.0, 0.0, -0.2])

        cam_pts = backproj(past_left_depth_th, inv_K)
        pixel_coords = reproject(cam_pts, K_th, cur_trans_mat_TA)

        past_left_normal_th = toTensor(past_left_normal)[None, ...].float()
        cur_left_restored_TA = F.grid_sample(past_left_normal_th, pixel_coords, padding_mode='zeros',
                                          align_corners=True)
        left_restored_TA = cur_left_restored_TA[0].permute(1, 2, 0).detach().cpu().numpy()
...
########################################
        ...
        rot_pred = scirot.from_matrix(gt_R)
        R_euler = rot_pred.as_euler('xyz', degrees=True)
...
##################################################
        ...
        # cam_pts = backproj(cur_right_depth_th, K_th)
        # pixel_coords = reproject(cam_pts, inv_K, cur_trans_mat)
        ...
        ax2.title.set_text('warped image (past2current)')
        ...
        plt.close()
        print(os.path.join('./output','%s_warp'%option,'%04d.png'%cnt))
